...
function initSubClean(subparsers, common) {
  ...
  parserClean.addArgument(['-k', '--keep'], {
    help: 'Remove obsolete versions and keep last KEEP ones. If value not provided, the last 5 will be kept.',
  });
...
if (args.command === 'fetch') {
  if (!args.tag || !args.force) {
    ...
    if (!args.force) while (1) {
      ...
      try {
        fs.lstatSync(path.join(args.dist, args.dirs.versions, tag));
      } catch (e) {
        args.tag = tag;
        break;
      }
